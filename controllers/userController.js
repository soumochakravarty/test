...
module.exports = function(app) {
    ...
    if (req.body.email == 'admin@admin.com') {
      bcrypt.genSalt(10, function(err, salt) {
        bcrypt.hash(body.password, salt, function(err, hash) {
          body.password = hash;
          User.query(
            "INSERT INTO `user` (`email`, `password`, `restaurant`) VALUES ('" +
              body.email +
              "', '" +
              body.password +
              "', '');",
      ...
      if (!body.password || !body.restaurant || !body.email || !body.table)
        res.sendStatus(400);
      else {
        bcrypt.genSalt(10, function(err, salt) {
          bcrypt.hash(body.password, salt, function(err, hash) {
            ...
            User.query(
              ...
              function(err, rows) {
                if (err) {
                  ...
                  Hoteltoken.query(
                    "SELECT tid FROM `Hoteltoken` WHERE restaurant ='" +
                      body.restaurant +
                      "'",
                    function(err, rows) {
                      if (err) throw err;
                      if (rows.length == 0) {
                        ...
                        Hoteltoken2.query(
                          'SELECT max(restaurantid) as restaurantid, max(buttonid) as buttonid FROM device_lookup;',
                          function(err, rows) {
                            if (err) throw err;
  ...
  app.post('/users/login', function(req, res) {
    ...
    User.query(
      "SELECT email, password,restaurant FROM user WHERE email='" +
        req.body.email +
        "';",
      function(err, rows) {
        if (err) throw err;
  ...
  app.post('/users/resetpwd', authenticate, function(req, res) {
    ...
    if (
      !body.password ||
      !body.password1 ||
      (sess.email != 'admin@admin.com' && body.email != sess.email) ||
  ...
  app.post('/users/deluser', authenticate, function(req, res) {
    var body = req.body;
    if (!body.email || !body.restaurant) res.sendStatus(400);
    else {
      User.query(
        "SELECT email FROM user WHERE restaurant='" + body.restaurant + "';",
        function(err, rows) {
          if (err) throw err;
          var length = rows.length;

          if (length == 0) res.sendStatus(400);
          else {
            if (length == 1) {
              if (body.email == rows[0].email) {
                User1.query(
                  "Delete FROM user WHERE restaurant='" +
                    body.restaurant +
                    "' and email='" +
                    body.email +
                    "';",
